...
IMPLEMENT_DYNAMIC(SGBM, CDialogEx)

const int imageWidth = 320;                      //【可能需要修改的程序参数1】：单目图像的宽度
const int imageHeight = 480;                      //【可能需要修改的程序参数2】：单目图像的高度
...
Mat cameraMatrixL = (Mat_<double>(3, 3) << 916.54068, 0, 673.20114,
	0, 916.48588, 369.25067,
	0, 0, 1);


Mat distCoeffL = (Mat_<double>(5, 1) << 0.01652, -0.02490, -0.00019, 0.00115, 0.00000);
...
0            0              1
-----------------------------------------*/
Mat cameraMatrixR = (Mat_<double>(3, 3) << 911.68855, 0, 621.85690,
	0, 912.25368, 386.33921,
	0, 0, 1);


Mat distCoeffR = (Mat_<double>(5, 1) << 0.02419, -0.04679, -0.00010, 0.00103, 0.00000);
//                                   [kc_right_01,  kc_right_02,  kc_right_03,  kc_right_04,   kc_right_05]


Mat T = (Mat_<double>(3, 1) << 200.30262, -0.20942, 1.92853);    //T平移向量
							 //[T_01,        T_02,       T_03]

Mat rec = (Mat_<double>(3, 1) << 0.02363, 0.00001, 0.01046);   //rec旋转向量
...
int SGBM:: main3()
{
	...
	//--【可能需要修改的程序参数5】：读取图片----------------------------------------------------------------
	rgbImageL = imread("Left1.bmp", CV_LOAD_IMAGE_COLOR);
	cvtColor(rgbImageL, grayImageL, CV_BGR2GRAY);
	rgbImageR = imread("Right1.bmp", CV_LOAD_IMAGE_COLOR);
	...
	imwrite("rectifyImageL.bmp", rectifyImageL); imwrite("rectifyImageR.bmp", rectifyImageR);
...
SGBM::SGBM(CWnd* pParent /*=nullptr*/)
...
void SGBM::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(SGBM, CDialogEx)
END_MESSAGE_MAP()
