...
#define Image_Width   320    //【可能需要修改的程序参数2】：定义单目输出图像的宽度，
#define Image_height  480     //【可能需要修改的程序参数3】：定义单目输出图像的高度


// catch_image 对话框

IMPLEMENT_DYNAMIC(catch_image, CDialogEx)

catch_image::catch_image(CWnd* pParent /*=nullptr*/)
...
int catch_image::yunxing(int argc, const char ** argv)          //程序主函数
{
	...
	while (1)
	{
		...
		if (key == 'R')       //当左目与右目图像显示在屏幕最前端时，按下大写字母R，则图像将被缩放后显示，再次按下则原比例显示。
		{
		...
		if (key == 'C')    //当左目与右目图像显示在屏幕最前端时，按下大写字母C，将左目与右目原始图像（非缩放图）抓拍并存储在Mian.cpp同级目录的文件夹。
		{
...
void catch_image::show_catch()
{

}


void catch_image::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(catch_image, CDialogEx)
END_MESSAGE_MAP()
